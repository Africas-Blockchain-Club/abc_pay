...
} from "../src/modules/settlement/settlement.worker.js";
...
describe("Settlement Module", () => {
  ...
  describe("Settlement Worker (settlement.worker.ts)", () => {
    it("processes pending settlements through BankPayoutProvider and records bankReference", async () => {
      ...
      vi.spyOn(prisma.settlement, "findMany").mockResolvedValue(pendingSettlements as any);
      const updateSpy = vi.spyOn(prisma.settlement, "update").mockResolvedValue({} as any);

      const mockProvider: BankPayoutProvider = {
