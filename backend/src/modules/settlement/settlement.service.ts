...
export interface MerchantPayoutCalculation {
  merchantId: string;
  totalNetZar: Prisma.Decimal;
  paymentIds: string[];
}
...
export class SettlementService {
  ...
  calculateMerchantPayouts(
    payments: Array<{
      ...
      amountZar: Prisma.Decimal;
      feeZar: Prisma.Decimal;
      merchantNetZar: Prisma.Decimal | null;
    }>
  ): MerchantPayoutCalculation[] {
    const merchantMap = new Map<string, { total: Prisma.Decimal; paymentIds: string[] }>();

    for (const payment of payments) {
      if (!payment.merchantId) continue;
      ...
      if (netAmount.isNegative() || netAmount.isZero()) continue;
