...
export async function alertOnDiscrepancy(runId, discrepancies) {
    // Stub for external alerts: PagerDuty / Slack / email
    console.warn(`[ALERT: Reconciliation Discrepancy] Run ID: ${runId} flagged ${discrepancies.length} discrepancy item(s).`, JSON.stringify(discrepancies, null, 2));
}
export class ReconciliationService {
     ...
     */
    matchRecords(ledgerEntries, custodyTransfers) {
        ...
        const matchedCustodyIds = new Set();
        for (const entry of ledgerEntries) {
            const ref = entry.reference?.trim();
            // Find match by reference or txHash
            const match = ref
                ? custodyTransfers.find((c) => !matchedCustodyIds.has(c.id) &&
     ...
     */
    async reconcileWindow(windowStart, windowEnd) {
        ...
        const { ledgerTotal, onChainTotal, discrepancyTotal, discrepancies } = this.matchRecords(ledgerEntries, custodyTransfers);
