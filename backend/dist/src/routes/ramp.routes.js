...
import { z } from "zod";
import { isValidSolanaAddress } from "../services/ramp.service.js";
...
export function createRampRouter(rampService) {
    const router = Router();
    /**
    ...
    router.post("/webhook/valr", async (req, res) => {
        try {
            const signature = req.headers["x-valr-signature"];
            const timestamp = req.headers["x-valr-timestamp"]
                ? parseInt(req.headers["x-valr-timestamp"], 10)
                : undefined;
