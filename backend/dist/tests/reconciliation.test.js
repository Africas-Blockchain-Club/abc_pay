...
describe("Reconciliation Module", () => {
    ...
    describe("End-to-End runReconciliation with seeded fixture data", () => {
        it("creates ReconciliationRun and ReconciliationDiscrepancy records on fixture set", async () => {
            ...
            vi.spyOn(prisma.ledgerEntry, "findMany").mockResolvedValue(fixtureLedger);
            vi.spyOn(prisma.custodyTransfer, "findMany").mockResolvedValue(fixtureCustody);
            const createRunSpy = vi.spyOn(prisma.reconciliationRun, "create").mockImplementation(async ({ data }) => {
                return {
                    id: "run_uuid_123",
                    ...data,
