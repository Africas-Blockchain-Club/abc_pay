...
import { shortenFloats, canonicalizeJson, computeSignatureV2, verifyDiditWebhookSignature, } from "../src/modules/kyc/kyc.webhook.js";
...
describe("Didit Webhook Signature & Canonicalization", () => {
    it("shortens whole floats and keeps non-whole floats", () => {
        ...
        const shortened = shortenFloats(input);
...
describe("POST /api/webhooks/didit HTTP endpoint", () => {
    ...
    it("accepts validly-signed sandbox payload and deduplicates replay", async () => {
        ...
        const mockProcessedEvents = new Set();
        vi.spyOn(prisma.processedWebhookEvent, "findUnique").mockImplementation(async ({ where }) => {
            return mockProcessedEvents.has(where.eventId) ? { eventId: where.eventId, processedAt: new Date() } : null;
        });
        vi.spyOn(prisma.processedWebhookEvent, "create").mockImplementation(async ({ data }) => {
            mockProcessedEvents.add(data.eventId);
            return { eventId: data.eventId, processedAt: new Date() };
        });
        const upsertVerificationMock = vi.spyOn(prisma.kycVerification, "upsert").mockResolvedValue({});
        const updateUserMock = vi.spyOn(prisma.user, "update").mockResolvedValue({});
